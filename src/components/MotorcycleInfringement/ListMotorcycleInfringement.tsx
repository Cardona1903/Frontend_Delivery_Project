...
import { deleteMotorcycleInfringement, getMotorcycleInfringements, updateMotorcycleInfringement } from "../../services/MotorcycleInfringementService";
...
const ListMotorcycleInfringements = () => {
  ...
  const handleView = (id: number) => {
    console.log(`Ver infracción de motocicleta con ID: ${id}`);
  };
  ...
  const handleSubmitEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        Swal.fire({
          title: "Actualizado",
          text: "La infracción de motocicleta ha sido actualizada correctamente",
          icon: "success",
        });
        ...
        Swal.fire({
          title: "Error",
          text: "No se pudo actualizar la infracción de motocicleta",
          icon: "error",
        });
      }
    } catch (error) {
      ...
      Swal.fire({
        title: "Error",
        text: "Ocurrió un error al actualizar la infracción de motocicleta",
        icon: "error",
      });
    }
  };
  ...
  const handleDelete = async (id: number) => {
    Swal.fire({
      title: "Eliminación",
      text: "¿Está seguro de querer eliminar esta infracción de motocicleta?",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        ...
        if (success) {
          Swal.fire("Eliminado", "La infracción de motocicleta ha sido eliminada", "success");
          fetchData();
        }
      }
    });
      ...
      <div className="flex flex-col gap-9">
        <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          <div className="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
            <h3 className="font-medium text-black dark:text-white">Listado de Infracciones de Motocicleta</h3>
          </div>
                ...
                <tbody>
                  {data.map((item) => (
                    <tr key={item.id} className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                      <td className="px-6 py-4 font-medium text-gray-900 dark:text-white">{new Date(item.date).toLocaleDateString()}</td>
      ...
      {isEditModalOpen && currentItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white dark:bg-boxdark p-6 rounded-lg w-full max-w-md">
            <h2 className="text-xl font-semibold mb-4 text-black dark:text-white">Editar Infracción de Motocicleta</h2>
